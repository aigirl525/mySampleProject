...
import com.example.lib_base.log.InternalLog
...
object AppManager {
    ...
    fun <A : Activity> getActivities1(clazz: Class<out A>): List<A?> {
        val list1 :MutableList<A?> = ArrayList()
        for( weakReferenceActivity in  activityStack){
            if (weakReferenceActivity.javaClass == clazz){
                list1.add(weakReferenceActivity as? A)
            }
        }
        return list1
